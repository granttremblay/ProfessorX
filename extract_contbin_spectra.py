...
def generate_spectra(path,obsids,onchip,regionfile):
    ...
    for obs in obsids:
        ...
        params = {'evt2': evtpath + str(obs) +  '_reproj_evt2.fits',
                  'bgevt2': evtpath + str(obs) + '_reproj_bgevt2.fits',
                  #'bpix': glob.glob(obsidpath + '/reprocess/acisf*_new_bpix1.fits')[0],
                  'bpix': glob.glob(obsidpath + '/reprocess/acisf*_bpix1.fits')[0],
            'reg': regionfile,
                  'spec': '%s_spec.pi' % name,
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print >>sys.stderr, "Usage %s path_to_obsids" % sys.argv[0]
        sys.exit(1)
